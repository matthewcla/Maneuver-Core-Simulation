...
const DASH_PATTERN_NONCAR = [
    4,
    4
]; // dashed
const DASH_PATTERN_SOLID = []; // solid
const LABEL_OFFSET_PX = 6; // gap between ring and label
const VECTOR_LINE_WIDTH = 3.36; // consistent width for all vectors
...
class ScenarioGenerator {
    ...
    makeScenario(ownship) {
        ...
        for(let i = 0; i < this.cfg.constraint_density; i++)tracks.push(this._makeHazard(ownship));
        return tracks;
    }
    _makePrimary(type, own) {
        ...
        switch(type){
            case 'STARBOARD_CROSS':
                ...
                crs = (own.course + 270 + this._rand(-10, 10)) % 360;
                break;
            case 'HEAD_ON':
                brg = this._rand(350, 10);
                crs = (own.course + 180 + this._rand(-5, 5)) % 360;
                break;
            case 'OVERTAKE':
                brg = this._rand(150, 210);
                crs = own.course + this._rand(-5, 5);
                spd = own.speed - this._rand(2, 4);
                break;
            case 'CONSTRAINED':
            default:
                brg = this._rand(40, 60);
                crs = (own.course + 270) % 360;
                break;
        }
    ...
    _tuneCPA(own, tgt) {
        for(let i = 0; i < 90; i++){
            ...
            const minT = this.cfg.time_to_cpa_range[0] / 60, maxT = this.cfg.time_to_cpa_range[1] / 60;
    ...
    _vel(v) {
        ...
        return {
            vx: v.speed * Math.cos(rad),
            vy: v.speed * Math.sin(rad)
        };
    }
    _rand(a, b) {
    ...
    generateTSS(level) {
        ...
        if (level >= 3) {
            ...
            for(let i = 0; i < numFollowing; i++){
                ...
                const y = this._rand(-8, -5); // Start South
    ...
    _spawnSimple(x, y, course, speed) {
        ...
        return {
            ...
            _base: {
                course,
                speed
            } // removed dummy initialBearing/Range to prevent overwrite in _initialize
        };
    }
}
class ContactController {
    ...
    update(dtHours, allContacts, cfg) {
        ...
        switch(this.t.state){
            ...
            case 'EXECUTING_MANEUVER':
                this._applyManeuver(dtHours);
                break;
            case 'RESUMING_COURSE':
                this._returnToBase(dtHours);
                break;
        }
    }
    ...
    _applyManeuver(dt) {
        ...
        if (Math.abs(diffC) > rateTurn) this.t.course = (this.t.course + Math.sign(diffC) * rateTurn + 360) % 360;
        else this.t.course = this.t._targetCourse;
        if (Math.abs(this.t.speed - this.t._targetSpeed) > acc) this.t.speed += Math.sign(this.t._targetSpeed - this.t.speed) * acc;
        else this.t.speed = this.t._targetSpeed;
        if (!this._collisionThreat([
            ...this.t._sim.tracks
        ], this.t._sim.scenarioCfg)) this.t.state = 'RESUMING_COURSE';
    }
    _returnToBase(dt) {
        ...
        if (Math.abs(this.t.speed - this.t._base.speed) > acc) this.t.speed += Math.sign(this.t._base.speed - this.t.speed) * acc;
 ...
 */ class Simulator {
    constructor(config = {}){
        ...
        if (!this.canvas) console.error('radarCanvas element not found in DOM');
        // console.log('Simulator init: 2D context:', this.ctx); // Reduced noise
        if (!this.ctx) console.error('Failed to get 2D context for radarCanvas');
    ...
    _initialize() {
        ...
        if (this.config.mode === 'gap_runner') this.loadGapRunnerScenario(1);
        else {
        ...
        if (this.tracks.length > 0) this.tracks.forEach((track)=>{
        ...
        if (this.isSimulationRunning) this.startGameLoop();
        if (document.fonts && document.fonts.ready) document.fonts.ready.then(()=>this.scaleUI());
    }
    loadGapRunnerScenario(level = 1) {
        ...
        if (level === 1) this.gapRunnerScore = 0;
        // UI Setup
    ...
    _attachEventListeners() {
        ...
        if (this.btnCommit) {
            ...
            this.btnCommit.addEventListener('touchstart', startCommit, {
                passive: false
            });
        ...
        if (this.helpModal && this.helpContent && this.helpCloseBtn) new ResizeObserver(()=>{
        ...
        const bindToggleSwitch = (chkId, toggleFn)=>{
            ...
            if (chk && toggleFn) chk.addEventListener('change', (e)=>{
        ...
        if (this.btnSettings && this.settingsDrawer) this.btnSettings.addEventListener('click', ()=>{
        ...
        document.addEventListener('keydown', (e)=>{
            if (e.key === 'Escape' && fsApi.isActive()) fsApi.exit();
        });
        ...
        document.querySelectorAll('[data-tooltip]').forEach((el)=>{
            el.addEventListener('pointerenter', (e)=>{
                ...
                if (e.clientX + tooltipWidth + buffer > window.innerWidth) x = e.clientX - tooltipWidth - buffer;
     ...
     */ fastForward() {
        ...
        if (this.simulationSpeed === this.ffSpeeds[this.ffSpeeds.length - 1]) this.simulationSpeed = 1;
        else {
            if (!this.isSimulationRunning) this.isSimulationRunning = true;
            const idx = this.ffSpeeds.indexOf(this.simulationSpeed);
            this.simulationSpeed = idx === -1 ? this.ffSpeeds[0] : this.ffSpeeds[idx + 1] || 1;
        }
        this.updateSpeedIndicator();
        this.updateButtonStyles();
        this.markSceneDirty();
     ...
     */ rewind() {
        // Cycle through –25×/–50× or reset to normal (1×)
        if (this.simulationSpeed === this.revSpeeds[this.revSpeeds.length - 1]) this.simulationSpeed = 1;
        else {
            if (!this.isSimulationRunning) this.isSimulationRunning = true;
            const idx = this.revSpeeds.indexOf(this.simulationSpeed);
            this.simulationSpeed = idx === -1 ? this.revSpeeds[0] : this.revSpeeds[idx + 1] || 1;
        }
        this.updateSpeedIndicator();
        this.updateButtonStyles();
        this.markSceneDirty();
    ...
    gameLoop(timestamp) {
        ...
        if (this.isSimulationRunning || this.sceneDirty) requestAnimationFrame(this.gameLoop);
        else this.gameLoop.running = false;
    }
    startGameLoop() {
    ...
    _renderEditableField(id, displayValue, numericValue) {
        ...
        } else if (el.textContent !== displayValue) el.textContent = displayValue;
    }
    /**
     * Format the numeric value for immediate static display after editing.
     */ _formatDisplayValue(id, num) {
        if (id === 'ownship-crs' || id === 'track-crs' || id === 'track-brg') return `${this.formatBearing(num)} T`;
        if (id === 'ownship-spd' || id === 'track-spd') return `${num.toFixed(1)} KTS`;
        if (id === 'track-rng') return `${num.toFixed(1)} NM`;
        return String(num);
    }
    /**
    ...
    commitEdit(id, value) {
        ...
        this.activeEditField = null;
        this._scheduleUIUpdate();
        if (didUpdate) this.markSceneDirty();
    }
    updateGapRunnerHUD() {
    ...
    updatePhysics(deltaTime) {
        ...
        if (Math.abs(courseDiff) <= maxTurn) this.ownShip.course = this.ownShip.orderedCourse;
        else this.ownShip.course = (this.ownShip.course + Math.sign(courseDiff) * maxTurn + 360) % 360;
        const maxSpdChange = 0.1 * dtSec; // knots per second
        const spdDiff = this.ownShip.orderedSpeed - this.ownShip.speed;
        if (Math.abs(spdDiff) <= maxSpdChange) this.ownShip.speed = this.ownShip.orderedSpeed;
        else this.ownShip.speed += Math.sign(spdDiff) * maxSpdChange;
    ...
    checkGapRunnerStatus(deltaTime) {
        ...
        for (const track of this.tracks){
            ...
            if (this.isCommitting && d < 1.0 && t > 0 && t < 0.2) {
                this.stopCommit();
            ...
            if (!this.perfectGapRegistry.has(track.id)) // Check if "past" CPA. t is time to CPA. If t < 0, CPA is in past.
    ...
    flashDamage() {
        ...
        // Trigger reflow
        this.hudElements.damageOverlay.offsetWidth;
    ...
    calculateAllData(track) {
        ...
        if (!track.rmVector) track.rmVector = {
            x: 0,
            y: 0,
            speed: 0,
            bearing: 0
        ...
        if (relSpeed < 0.001) {
            ...
            if (!track.cpaPosition) track.cpaPosition = {
                x: 0,
                y: 0
            };
        ...
        track.rm.rate = this.getBearingRate({
            x: relVelX,
            y: relVelY
        }, {
            x: targetPosX,
    ...
    drawRadar() {
        ...
        if (this.showWeather) this.drawWeatherInfo(center, radius);
        // --- TSS Rendering ---
        if (this.tssData) {
            this.drawTSS(center, radius);
            if (this.tssData.waypoint) this.drawWaypoint(center, radius);
        }
        this.drawOwnShipIcon(center, radius);
        this.tracks.forEach((track)=>{
            if (track.range > this.maxRange) return;
            this.drawTarget(center, radius, track);
            if (this.showRelativeMotion) this.drawRelativeMotionVector(center, radius, track);
        });
        if (this.showCPAInfo && this.selectedTrackId !== null) this.drawCPAIndicator(center, radius);
        ...
        if (this.hoveredTrackId !== null && this.hoveredTrackId !== this.selectedTrackId) this.drawSelectionIndicator(center, radius, this.hoveredTrackId, this.radarFaintWhite, 1);
    }
    ...
    drawStaticRadar() {
        ...
        if (this.showPolarPlot) for(let deg = 0; deg < 360; deg += 10){
            ...
            if (!isCardinal) endRadius = radius + 0.8 * (originalRadius - radius);
            ctx.beginPath();
            ctx.moveTo(center + startRadius * Math.cos(ang), center - startRadius * Math.sin(ang));
            ctx.lineTo(center + endRadius * Math.cos(ang), center - endRadius * Math.sin(ang));
            ctx.stroke();
        }
        ctx.restore();
    }
    ...
    drawOwnShipIcon(center, radius) {
        ...
        if (diffCourse > 0.5 || diffSpeed > 0.05) {
            ...
            this.ownShip.orderedVectorEndpoint = {
                x: oEndX,
                y: oEndY
            };
            this.ctx.save();
    ...
    drawTarget(center, radius, track) {
        ...
        if (this.showTrackIds) this.ctx.fillText(track.id, x + targetSize / 2 + 3, y + targetSize / 2 + 3);
    }
    ...
    drawSelectionIndicator(center, radius, trackId, color, lineWidth) {
        const track = this.tracks.find((t)=>t.id === trackId);
        if (!track) return;
        const { x, y } = this.getTargetCoords(center, radius, track);
        const targetSize = Math.max(11, radius * 0.038);
        const indicatorRadius = targetSize * 1.2;
        this.ctx.strokeStyle = color;
        this.ctx.lineWidth = lineWidth;
        this.ctx.beginPath();
        this.ctx.arc(x, y, indicatorRadius, 0, 2 * Math.PI);
        this.ctx.stroke();
    }
    ...
    updateDataPanels() {
        ...
        const updateToggleState = (chkId, isOn)=>{
            const chk = document.getElementById(chkId);
            if (chk && chk.checked !== isOn) chk.checked = isOn;
        };
        const showRM = selectedTrack && this.showRelativeMotion;
        updateToggleState('chk-toggle-rm', this.showRelativeMotion);
        // Visual disabled state
        if (this.rmDataContainer) this.rmDataContainer.classList.toggle('disabled', !this.showRelativeMotion);
        ...
        if (this.cpaDataContainer) this.cpaDataContainer.classList.toggle('disabled', !this.showCPAInfo);
    ...
    updateSpeedIndicator() {
        ...
        if (this.simulationSpeed > 1) {
            const label = this.simulationSpeed === 25 ? '25x' : this.simulationSpeed === 50 ? '50x' : `${this.simulationSpeed}x`;
            ...
            const label = absSpeed === 25 ? '25x' : absSpeed === 50 ? '50x' : `${absSpeed}x`;
    ...
    updateDragTooltip(e) {
        ...
        if (this.dragType === 'icon') {
            ...
            if (vessel) {
                const startPoint = vessel.id === 'ownShip' ? {
                    x: center,
                    y: center
                } : this.getTargetCoords(center, center * 0.9, vessel);
            ...
            if (this.dragType === 'windDirection') tooltipText = `Dir: ${this.formatBearing(this.trueWind.direction)} T`;
            else if (this.dragType === 'windSpeed') tooltipText = `Spd: ${this.trueWind.speed.toFixed(1)} kts`;
        }
        if (tooltipText) {
            if (this.draggedItemId === 'ownShip' && this.dragType === 'vector') this.dragTooltip.style.color = this.radarWhite;
            else this.dragTooltip.style.color = this.radarGreen;
        ...
        } else this.dragTooltip.style.display = 'none';
    }
    handlePointerDown(e) {
        if (e.button !== 0 && e.buttons !== undefined && e.buttons !== 1) return;
        if (this.canvas.setPointerCapture && e.pointerId !== undefined) try {
        ...
        if (item) {
            ...
            if (item.id !== 'ownShip' && item.id !== 'trueWind') this.selectedTrackId = item.id;
    ...
    handlePointerUp(e) {
        if (this.canvas.releasePointerCapture && e && e.pointerId !== undefined) try {
    ...
    handlePointerMove(e) {
        ...
        if (this.draggedItemId) {
            ...
            if (this.draggedItemId === 'trueWind') {
                ...
                const dy = -(mouseY - center);
                const newCanvasAngleRad = Math.atan2(dy, dx);
                if (this.dragType === 'windDirection') this.trueWind.direction = this.canvasAngleToBearing(this.toDegrees(newCanvasAngleRad));
    ...
    getInteractiveItemAt(mouseX, mouseY) {
        ...
        for (const track of this.tracks){
            ...
            if (mouseX > x - size / 2 && mouseX < x + size / 2 && mouseY > y - size / 2 && mouseY < y + size / 2) return {
                type: 'icon',
                id: track.id
            };
        }
        ...
        for (const vessel of allVessels){
            ...
            const startPt = vessel.id === 'ownShip' ? {
                x: center,
                y: center
            } : this.getTargetCoords(center, radius, vessel);
    ...
    togglePlayPause() {
        ...
        this.updateButtonStyles();
        this.updateSpeedIndicator();
        if (!wasRunning) this.startGameLoop();
    }
    fastForward() {
        const cycle = [
            1,
    ...
    async toggleFullScreen() {
        try {
            if (!fsApi.isActive()) await fsApi.request();
    ...
    bearingToCanvasAngle(bearing) {
        return (450 - bearing) % 360;
    }
    canvasAngleToBearing(angle) {
    ...
    getBearingRate(relativeVelocity, targetPosition, range) {
        ...
        let direction;
        if (Math.abs(bearingRateDpm) < 0.01) direction = 'STEADY';
        else if (bearingRateDpm > 0) direction = 'LEFT';
...
//# sourceMappingURL=Maneuver-Core-Simulation.e36cf8d6.js.map
